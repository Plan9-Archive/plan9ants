...
usage(void)
{
	...
	fprint(2, "\tcfs [-a netaddr | -F srv] [-dknrS] [-f partition] [-z srvname] [mntpt]\n");
	exits("usage");
}
...
{
	int std, svc, format, chkid;
	char *part, *server, *mtpt, *srvice;
	NetConnInfo *snci;
	...
	case 's':
		...
		break;
	case 'z':
		srvice = smprint("/srv/%s", EARGF(usage()));
		svc = 1;
		break;		
	default:
		usage();
...
void
svcinit(char *server, char *srvice)
{
	int p[2];
	int fd;
...
sendmsg(P9fs *p, Fcall *f)
{
	...
	if(write(p->fd[1], datasnd, p->len)!=p->len)
		error("sendmsg error to %s fd 1", p->name);
}
